...
for t=1:nRlz
    ...
    observed_std  = zeros(prod(dim),1);
      ...
      for k=1:nBoot 
          ...
          boot_mean                         = mean(boundary_resid_field, 2);
          boot_std                          = std(boundary_bootstrap, 0, 2);
          boot_cohen_d                      = boot_mean./boot_std;
          boundary_resid_field              = boundary_resid_field./sqrt(1+boot_cohen_d.^2/2);
          ...
          % Re-standardizing by bootstrap standard deviation
          observed_boot_mean                = mean(observed_boundary_resid_field, 2);
          observed_boot_std                 = std(observed_boundary_bootstrap, 0, 2);
          observed_boot_cohen_d             = observed_boot_mean./observed_boot_std;
          observed_boundary_resid_field     = observed_boundary_resid_field./sqrt(1+observed_boot_cohen_d.^2/2);
