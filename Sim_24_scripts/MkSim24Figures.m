        ...
        for v = 1:length(dim)
            for x = 1:length(distribution_type_boot)
                for y = 1:length(boot_estimator)
                    ...
                    fh = gcf;
                    set(fh,'color','w');
                    SvNm = sprintf('%s_%s_%s_%s_square_dim_%s_effect_%s_smo.pdf', boot_type_SvNm(z), boot_estimator_SvNm(y), distribution_type_SvNm(x), num2str(dim(v)), num2str(erase(num2str(mag(j)),'.')), num2str(smo(k)));
                    export_fig(fh, fullfile(figures_dir, SvNm))
                    end 
                end
            end
        end    
