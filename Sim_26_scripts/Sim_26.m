...
non_stationary_sd = spm_vol(fullfile(Biobank_data_dir,'fullstd_BOLD.nii'));
non_stationary_sd = spm_read_vols(non_stationary_sd);
...
for t=1:nRlz
    ...
    observed_std  = zeros(prod(dim),1);
      ...
      observed_boundary_edges   = getBdryparams(reshape(observed_cohen_d, dim), (1 - (3/(4*nSubj - 1)))^(-1)*thr);
