...
for t=1:nRlz
    ...
    observed_std  = zeros(prod(dim),1);
      ...
      cohen_resid_mean = zeros(prod(dim),1);
      cohen_resid_std  = zeros(prod(dim),1);
      for i=1:nSubj
          cohen_resid = create_resid(observed_data(:,i), observed_mean, observed_std, 2);
          cohen_resid_mean = cohen_resid_mean + cohen_resid;
          cohen_resid_std = cohen_resid_std + cohen_resid.^2;
          subject_resid_field                 = reshape(cohen_resid, dim);
          resid_boundary_values(:,i)          = get_int_ext_Bdryvalues(subject_resid_field, Sig_boundary_edges);
          observed_resid_boundary_values(:,i) = get_int_ext_Bdryvalues(subject_resid_field, observed_boundary_edges);
      end
      
      cohen_resid_std = sqrt(cohen_resid_std/nSubj - cohen_resid_mean.^2);
      cohen_resid_std = reshape(cohen_resid_std, dim);
      monte_carlo_cohen_d_boundary_values(:,t) = get_int_ext_Bdryvalues(sqrt(nSubj)*(reshape(observed_cohen_d, dim) - thr)./cohen_resid_std, Sig_boundary_edges);
      cohen_resid_std_boundary_values = get_int_ext_Bdryvalues(cohen_resid_std, Sig_boundary_edges);
      observed_cohen_resid_std_boundary_values = get_int_ext_Bdryvalues(cohen_resid_std, observed_boundary_edges);
      resid_boundary_values = resid_boundary_values./cohen_resid_std_boundary_values;
      observed_resid_boundary_values = observed_resid_boundary_values./observed_cohen_resid_std_boundary_values;
      
      
      % Implementing the Multiplier Boostrap to obtain confidence intervals
      for k=1:nBoot 
          % Applying the bootstrap using Rademacher variables (gauss)
          gauss                              = normrnd(0,1,[nSubj,1]);

          % True boundary
          boundary_bootstrap                = resid_boundary_values*spdiags(gauss, 0, nSubj, nSubj);
          ...
          % Estimated boundary
          observed_boundary_bootstrap       = observed_resid_boundary_values*spdiags(gauss, 0, nSubj, nSubj);
    ...
    lower_contour_raw_80             = observed_cohen_d >= thr - supGa_raw_80*tau*cohen_resid_std;
    upper_contour_raw_80             = observed_cohen_d >= thr + supGa_raw_80*tau*cohen_resid_std;
    ...
    lower_contour_raw_90             = observed_cohen_d >= thr - supGa_raw_90*tau*cohen_resid_std;
    upper_contour_raw_90             = observed_cohen_d >= thr + supGa_raw_90*tau*cohen_resid_std;
    ...
    lower_contour_raw_95             = observed_cohen_d >= thr - supGa_raw_95*tau*cohen_resid_std;
    upper_contour_raw_95             = observed_cohen_d >= thr + supGa_raw_95*tau*cohen_resid_std;
    ...
    max_min_lower_contour_raw_80             = observed_cohen_d >= thr + supGa_min_raw_80*tau*cohen_resid_std;
    max_min_upper_contour_raw_80             = observed_cohen_d >= thr + supGa_max_raw_80*tau*cohen_resid_std;
    ...
    max_min_lower_contour_raw_90             = observed_cohen_d >= thr + supGa_min_raw_90*tau*cohen_resid_std;
    max_min_upper_contour_raw_90             = observed_cohen_d >= thr + supGa_max_raw_90*tau*cohen_resid_std;
    ...
    max_min_lower_contour_raw_95             = observed_cohen_d >= thr + supGa_min_raw_95*tau*cohen_resid_std;
    max_min_upper_contour_raw_95             = observed_cohen_d >= thr + supGa_max_raw_95*tau*cohen_resid_std;
    ...
    lower_contour_observed_80             = observed_cohen_d >= thr - supGa_observed_80*tau*cohen_resid_std;
    upper_contour_observed_80             = observed_cohen_d >= thr + supGa_observed_80*tau*cohen_resid_std;
    ...
    lower_contour_observed_90             = observed_cohen_d >= thr - supGa_observed_90*tau*cohen_resid_std;
    upper_contour_observed_90             = observed_cohen_d >= thr + supGa_observed_90*tau*cohen_resid_std;
    ...
    lower_contour_observed_95             = observed_cohen_d >= thr - supGa_observed_95*tau*cohen_resid_std;
    upper_contour_observed_95             = observed_cohen_d >= thr + supGa_observed_95*tau*cohen_resid_std;
    ...
    max_min_lower_contour_observed_80             = observed_cohen_d >= thr + supGa_min_observed_80*tau*cohen_resid_std;
    max_min_upper_contour_observed_80             = observed_cohen_d >= thr + supGa_max_observed_80*tau*cohen_resid_std;
    ...
    max_min_lower_contour_observed_90             = observed_cohen_d >= thr + supGa_min_observed_90*tau*cohen_resid_std;
    max_min_upper_contour_observed_90             = observed_cohen_d >= thr + supGa_max_observed_90*tau*cohen_resid_std;
    ...
    max_min_lower_contour_observed_95             = observed_cohen_d >= thr + supGa_min_observed_95*tau*cohen_resid_std;
    max_min_upper_contour_observed_95             = observed_cohen_d >= thr + supGa_max_observed_95*tau*cohen_resid_std;
    ...
    lower_condition_80 = thr - supGa_raw_80*tau*cohen_resid_std;
    upper_condition_80 = thr + supGa_raw_80*tau*cohen_resid_std;
    lower_condition_90 = thr - supGa_raw_90*tau*cohen_resid_std;
    upper_condition_90 = thr + supGa_raw_90*tau*cohen_resid_std;
    lower_condition_95 = thr - supGa_raw_95*tau*cohen_resid_std;
    upper_condition_95 = thr + supGa_raw_95*tau*cohen_resid_std;
    ...
    max_min_lower_condition_80 = thr + supGa_min_raw_80*tau*cohen_resid_std;
    max_min_upper_condition_80 = thr + supGa_max_raw_80*tau*cohen_resid_std;
    max_min_lower_condition_90 = thr + supGa_min_raw_90*tau*cohen_resid_std;
    max_min_upper_condition_90 = thr + supGa_max_raw_90*tau*cohen_resid_std;
    max_min_lower_condition_95 = thr + supGa_min_raw_95*tau*cohen_resid_std;
    max_min_upper_condition_95 = thr + supGa_max_raw_95*tau*cohen_resid_std;
    ...
    lower_condition_80_observed = thr - supGa_observed_80*tau*cohen_resid_std;
    upper_condition_80_observed = thr + supGa_observed_80*tau*cohen_resid_std;
    lower_condition_90_observed = thr - supGa_observed_90*tau*cohen_resid_std;
    upper_condition_90_observed = thr + supGa_observed_90*tau*cohen_resid_std;
    lower_condition_95_observed = thr - supGa_observed_95*tau*cohen_resid_std;
    upper_condition_95_observed = thr + supGa_observed_95*tau*cohen_resid_std;
    ...
    max_min_lower_condition_80_observed = thr + supGa_min_observed_80*tau*cohen_resid_std;
    max_min_upper_condition_80_observed = thr + supGa_max_observed_80*tau*cohen_resid_std;
    max_min_lower_condition_90_observed = thr + supGa_min_observed_90*tau*cohen_resid_std;
    max_min_upper_condition_90_observed = thr + supGa_max_observed_90*tau*cohen_resid_std;
    max_min_lower_condition_95_observed = thr + supGa_min_observed_95*tau*cohen_resid_std;
    max_min_upper_condition_95_observed = thr + supGa_max_observed_95*tau*cohen_resid_std;
