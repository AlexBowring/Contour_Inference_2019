...
transformed_thr = alpha*asinh(thr*beta);
...
transformed_Sig = alpha*asinh(Sig*beta);

% Uncomment to look at the Signal
%imagesc(Sig); axis image; colorbar
AC                      = transformed_Sig >= transformed_thr;
middle_contour          = AC;
middle_contour_volume   = sum(middle_contour(:));

transformed_Sig_boundary_edges   = getBdryparams(transformed_Sig, transformed_thr);
transformed_n_Sig_boundary_edges = size(getBdryvalues(transformed_Sig, transformed_Sig_boundary_edges),1);
...
for t=1:nRlz
    ...
    observed_std  = zeros(prod(dim),1);
      ...
      transformed_observed_boundary_edges   = getBdryparams(reshape(transformed_observed_cohen_d, dim), transformed_thr);
      transformed_n_observed_boundary_edges = size(getBdryvalues(transformed_Sig, transformed_observed_boundary_edges),1);
      
      resid_boundary_values = zeros([transformed_n_Sig_boundary_edges nSubj]);
      observed_resid_boundary_values = zeros([transformed_n_observed_boundary_edges nSubj]);
      ...
      for i=1:nSubj
          ...
          resid_boundary_values(:,i)          = getBdryvalues(subject_resid_field, transformed_Sig_boundary_edges);
          observed_resid_boundary_values(:,i) = getBdryvalues(subject_resid_field, transformed_observed_boundary_edges);
      end
      
      mammen = zeros(nSubj,1);
      % Implementing the Multiplier Boostrap to obtain confidence intervals
      for k=1:nBoot 
          % Applying the bootstrap using Mammen variables 
          uniform                            = rand(nSubj,1);
          mammen(uniform < (sqrt(5)+1)/(2*sqrt(5))) = -(sqrt(5) - 1)/2;
          mammen(uniform > (sqrt(5)+1)/(2*sqrt(5))) = (sqrt(5) + 1)/2;
          
          % True boundary
          boundary_bootstrap                = resid_boundary_values*spdiags(mammen, 0, nSubj, nSubj);
          ...
          % Estimated boundary
          observed_boundary_bootstrap       = observed_resid_boundary_values*spdiags(mammen, 0, nSubj, nSubj);
    ...
    transformed_observed_cohen_d_true_boundary_values = getBdryvalues(transformed_observed_cohen_d, transformed_Sig_boundary_edges);
